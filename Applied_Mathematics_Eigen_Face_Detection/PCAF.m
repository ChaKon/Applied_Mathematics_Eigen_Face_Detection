function[eigVec,MDT]=PCAF(file,pathname)
...
%     Grayscaleimage=rgb2gray(a);% coverting image in Training set to gray scale
    Mt(i,:)=reshape(a',1,((size(a,1)*size(a,2)))); % converting image into vector form
end
p=size(file,1); 
Sigma_prime=(1/(p-1))*Mt'*Mt;% covariance matices
k=90;
[eigVec eigval]=eigs(Sigma_prime,k); % computing eigen vectors and eigen values
